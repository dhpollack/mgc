...
with open("audioset/unbalanced_train_segments_cropped.csv", "r") as f:
    csvreader = csv.reader(f, doublequote=True, skipinitialspace=True)
    segments = [row for row in csvreader]
    print(segments[:5])
    segments = [(idx, st, fin, tags.split(",")) for idx, st, fin, tags in segments]
...
if args.download:
    ...
    for raw_yt_fn in raw_yt_files:
        ...
        cmd = "ffmpeg -n -ss {} -t {} -i {} -acodec pcm_s16le -ac 1 -ar 16000 {}".format(st, dur, infile, outfile)
        #print(cmd)
        os.system(cmd)
